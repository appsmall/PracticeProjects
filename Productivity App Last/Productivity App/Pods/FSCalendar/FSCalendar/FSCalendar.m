    ...
    if (_needsAdjustingViewFrame) {
        ...
        if (!self.floatingMode) {
            switch (self.transitionCoordinator.representingScope)
            {
                ...
                {
                    self.calendarHeaderView.frame = CGRectMake(0, 0 + 8, self.fs_width, headerHeight + 10);
                    self.calendarWeekdayView.frame = CGRectMake(0, self.calendarHeaderView.fs_bottom + 10, self.contentView.fs_width, weekdayHeight + 8);
                    
                    CGFloat contentHeight = rowHeight + padding;
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight+55, self.fs_width, contentHeight*2);
                    _collectionView.frame = CGRectMake(0, 0, _daysContainer.fs_width, contentHeight+5);
                    //_collectionView.backgroundColor = UIColor.redColor;
                    
                    FSCalendarAppearance *appearance = [[FSCalendarAppearance alloc] init];
                    appearance.calendar.appearance.titleFont = appearance.calendar.appearance.titleFont;
...
- (CGFloat)preferredRowHeight
{
    if (_preferredRowHeight == FSCalendarAutomaticDimension) {
        ...
        if (!self.floatingMode)
        {
            ...
            if (_transitionCoordinator.representingScope == FSCalendarScopeWeek) {
                _preferredRowHeight = (contentHeight-padding*2)/3.0;
                NSLog(@"WeekDayScope");
            }
            else {
